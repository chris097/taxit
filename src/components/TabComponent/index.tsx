  ...
  return (
    <AnimateSlider>
      <div className='py-32 md:flex justify-between items-center'>
          <div className='md:h-[444px] h-[600px] md:w-[43%] border-l-[6px] border-[#DBE2EA] relative'>
            {tabs.map((tab, index) => (
              <div onClick={() => setActiveIndex(index)} key={index}>
                {activeIndex === index && <div className='border-l-[6px] border-main-3 md:h-[81px] h-[120px] absolute -left-[5.3px]' />}
            <div className='h-[81px] pl-5 md:mb-10 mb-20 flex text-main-10 flex-col justify-center'>
                  <h1 className='md:text-[28px] text-2xl leading-[44px] font-semibold'>{tab.title}</h1>
                  <p className='md:text-base text-sm mt-2 leading-[28.8px]'>{tab.description}</p>
            </div>
              </div>
            ))}
          </div>
